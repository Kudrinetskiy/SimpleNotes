...
+ (instancetype)sharedManager {
    ...
    dispatch_once(&token, ^{ shared = [[super alloc] init]; } );
    return shared;
}
...
- (NSString *)fileName {
    ...
    return [NSString stringWithFormat:@"%@/notes.data", cachePath];
}
...
-(void)saveToFile {
    ...
    [frozenInfo writeToFile:[self fileName] atomically:YES];
}
...
-(void)addOrUpdateNote:(PSRNote *)note {
    ...
    if (noteToUpdate) {
        ...
        noteToUpdate.textColor = note.textColor;
